...
def get_cols(player_type):
    if player_type == 'batter':
        cols = ['Player', 'RISP', 'H', 'HR%', 'AVG', 'OBP', 'SLG', 'OPS', 'R', 'RBI', 'SB']
    else:
        cols = ['Player', 'W/L/SV', 'IP', 'ERA', 'WHIP', 'HR/9', 'SO', 'K/9', 'B/9', 'RA']
...
def fix_pitcher_types(pitcher_data):
    pitcher_data[['W', 'L', 'SV']] = pitcher_data['W/L/SV'].str.split('-').apply(pd.Series)
    pitcher_data = pitcher_data[['Player', 'W', 'L', 'SV', 'IP', 'ERA', 'WHIP', 'HR/9', 'SO', 'K/9', 'B/9', 'RA']]
...
st.title('9 Innings OVR Analysis')
st.markdown('Upload screenshots from OVR Analysis stats and this tool will aggregate the data into a single table.')
st.warning('This has only been tested with screenshots from an iPhone 13. Screenshots from other devices may or may not be compatible.')
...
if len(pitcher_data):
    ...
    avg_pitcher_stats = pitcher_data.groupby('Player').mean().sort_values('ERA', ascending=True)
    avg_pitcher_stats

if len(batter_data):
    st.header('Average Batter Stats')
    avg_batter_stats = batter_data.groupby('Player').mean().sort_values('OPS', ascending=False)
    avg_batter_stats
